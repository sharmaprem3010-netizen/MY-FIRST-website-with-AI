...
export default function App() {
  ...
  const navigateTo = (page) => { 
    ...
    setActivePage(page); 
    if (window.innerWidth < 1024) setIsSidebarHidden(true);
    // Reset scroll of the main container if possible
  ...
  const generateStudyPlan = async (subject, grade) => {
    ...
    try {
      const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
      const prompt = `Create a 7-day intensive study plan for a ${grade} student preparing for ${subject}. Format using HTML tags (like <b>, <ul>, <li>, <h3>) so it renders beautifully in a web page. Make it highly actionable. Keep it concise.`;
      const result = await model.generateContent(prompt);
  ...
  const renderHome = () => (
    <div className="animate-fade-in w-full flex flex-col items-center">
      <div className="text-center relative z-10 mb-12 w-full max-w-4xl mx-auto">
        ...
        <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-[5.5rem] font-black mb-6 tracking-tight leading-none drop-shadow-2xl text-white break-words">
        ...
        <p className="text-base sm:text-lg md:text-xl text-gray-400 leading-relaxed font-light px-4">
          The all-in-one spatial educational universe. Enter the simulator, decode your syllabus, and let Nova AI guide your path.
        </p>
      </div>

      <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 pb-32">
        {NAV_SYSTEMS.slice(1, 7).map((system, idx) => (
          <div key={idx} onClick={() => navigateTo(system.id)} className="group relative cursor-pointer flex flex-col w-full">
            ...
            <div className="relative flex-1 bg-gray-900/80 backdrop-blur-xl border border-gray-700/50 rounded-3xl p-6 md:p-8 transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-[0_15px_30px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col w-full items-start">
              ...
              <div className="relative z-10 flex-1 flex flex-col w-full">
                <h2 className="text-2xl md:text-3xl font-bold mb-3 text-white tracking-tight break-words">{system.label}</h2>
  ...
  const renderAcademics = () => (
    <div className="w-full animate-fade-in relative">
      <div className="flex flex-wrap items-center gap-4 mb-10 border-b border-gray-800 pb-6 w-full">
      ...
      <div className="space-y-8 w-full">
        {/* Selection Step 1 */}
        <div className="w-full bg-gray-900/60 backdrop-blur-md border border-gray-800 p-6 md:p-8 rounded-3xl shadow-xl">
        ...
        {academicsParams.board && (
          <div className="w-full bg-gray-900/60 backdrop-blur-md border border-gray-800 p-6 md:p-8 rounded-3xl shadow-xl animate-fade-in">
        ...
        {academicsParams.subject && (
          <div className="pt-6 animate-fade-in w-full">
            <div className="w-full bg-gradient-to-r from-gray-900 to-gray-800 border border-gray-700 p-6 md:p-10 rounded-3xl shadow-2xl">
                  ...
                  {syllabusData.filter(s => s.board === academicsParams.board && s.subject === academicsParams.subject).length > 0 ? (
                    <ul className="space-y-4 text-gray-300 w-full">
                      {syllabusData.filter(s => s.board === academicsParams.board && s.subject === academicsParams.subject).map(item => (
                         <li key={item.id} className="w-full bg-gray-800/80 border border-gray-700 p-5 md:p-6 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-colors hover:border-blue-500">
                            <div className="w-full min-w-0"><strong className="text-lg md:text-xl text-blue-300 break-words block">{item.chapter}</strong><p className="text-sm md:text-base mt-2 text-gray-400 break-words">{item.content}</p></div>
                    ...
                    <div className="w-full text-center py-12 text-gray-500 border border-dashed border-gray-700 rounded-3xl bg-gray-900/30">
  ...
  const renderAI = () => (
    <div className="w-full animate-fade-in flex flex-col h-full min-h-[75vh]">
      <div className="flex flex-wrap items-center gap-4 mb-6 border-b border-gray-800 pb-4 shrink-0 w-full">
      ...
      <div className="w-full flex-1 bg-gray-900/80 border border-gray-700/50 rounded-3xl overflow-hidden flex flex-col backdrop-blur-2xl shadow-[0_0_50px_rgba(6,182,212,0.1)] relative">
        <div className="bg-gradient-to-b from-gray-800/80 to-transparent p-6 border-b border-gray-800/50 flex flex-col items-center justify-center shrink-0 w-full">
        ...
        <div className="w-full flex-1 p-4 md:p-8 overflow-y-auto space-y-6 scroll-smooth">
          {messages.map((m, i) => (
        ...
        <div className="w-full p-4 md:p-6 bg-gray-900 border-t border-gray-800 z-20 shrink-0">
          <form onSubmit={handleSendMessage} className="flex gap-2 md:gap-4 w-full max-w-5xl mx-auto">
            <input 
              ...
              className="w-full flex-1 bg-gray-800/80 border border-gray-700 rounded-xl md:rounded-2xl px-4 py-3 md:px-6 md:py-4 text-sm md:text-lg text-white focus:outline-none focus:border-cyan-500 transition-all shadow-inner"
  ...
  const renderExams = () => (
    <div className="w-full animate-fade-in">
      <div className="w-full flex flex-wrap items-center gap-4 mb-10 border-b border-gray-800 pb-6">
        <span className="text-4xl md:text-5xl">🎯</span>
        <div className="flex-1 min-w-[200px]">
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10">
        ...
        {/* Quiz Box */}
        <div className="w-full group relative flex flex-col">
          ...
          <div className="w-full relative flex-1 bg-gray-900/80 border border-gray-800 p-6 md:p-10 rounded-3xl backdrop-blur-xl transition-all duration-300 group-hover:-translate-y-2 group-hover:border-green-500/50 shadow-2xl flex flex-col justify-between">
        ...
        <div className="w-full group relative flex flex-col">
          ...
          <div className="w-full relative flex-1 bg-gray-900/80 border border-gray-800 p-6 md:p-10 rounded-3xl backdrop-blur-xl transition-all duration-300 group-hover:-translate-y-2 group-hover:border-yellow-500/50 shadow-2xl flex flex-col justify-between">
  ...
  const renderActiveQuiz = () => {
    ...
    return (
      <div className="w-full max-w-4xl mx-auto py-8 md:py-12 animate-fade-in">
        ...
        <div className="w-full bg-gray-900 border border-gray-700/50 p-6 md:p-10 rounded-3xl shadow-2xl backdrop-blur-xl">
          <h2 className="text-xl md:text-3xl font-black mb-8 md:mb-10 leading-snug text-white break-words">{currentQ.question}</h2>
          <div className="w-full grid grid-cols-1 gap-4 w-full">
  ...
  const renderActiveFlashcards = () => {
    ...
    return (
      <div className="w-full max-w-5xl mx-auto py-8 md:py-10 animate-fade-in text-center flex flex-col items-center min-h-[70vh] justify-center">
        <h2 className="w-full text-2xl md:text-4xl font-black mb-4 text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-500">Neural Deck Online</h2>
        ...
        <div className="w-full max-w-2xl min-h-[300px] md:min-h-[350px] mb-10 cursor-pointer group relative" onClick={() => setIsFlipped(!isFlipped)}>
          {!isFlipped ? (
        ...
        <div className="w-full max-w-2xl flex flex-col sm:flex-row gap-3 sm:gap-6">
          <button 
            disabled={flashcardIndex === 0} 
            onClick={() => { setFlashcardIndex(prev => prev - 1); setIsFlipped(false); }} 
            className="w-full flex-1 bg-gray-800 hover:bg-gray-700 disabled:opacity-30 border border-gray-700 text-white py-4 rounded-xl font-bold text-sm md:text-lg transition-all shadow-lg">
            ← Previous
          </button>
          ...
          {flashcardIndex === activeFlashcards.cards.length - 1 ? (
             <button 
               ...
               className="w-full flex-1 bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400 text-gray-900 py-4 rounded-xl font-black text-sm md:text-lg transition-all shadow-lg">
               COMPLETE DECK 🏆
             </button>
          ) : (
             <button 
               onClick={() => { setFlashcardIndex(prev => prev + 1); setIsFlipped(false); }} 
               className="w-full flex-1 bg-gray-700 hover:bg-gray-600 border border-gray-600 text-white py-4 rounded-xl font-bold text-sm md:text-lg transition-all shadow-lg">
               Next →
             </button>
          )}
        </div>
  ...
  const renderNotebook = () => (
    <div className="w-full animate-fade-in">
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8 w-full">
        {/* Editor */}
        <div className="w-full lg:col-span-1 bg-gray-900/80 border border-gray-800 rounded-3xl p-6 md:p-8 backdrop-blur-xl h-fit lg:sticky top-6 shadow-2xl flex flex-col">
          ...
          <div className="w-full flex flex-col gap-3 md:gap-4">
            ...
            <div className="w-full flex gap-3 md:gap-4">
              ...
              <button onClick={simplifyNoteWithAI} disabled={isGenerating} className="w-full flex-1 bg-gray-800 hover:bg-gray-700 border border-teal-500/30 text-white font-bold py-3 md:py-4 rounded-xl transition-all flex justify-center items-center gap-2 group text-xs md:text-sm" title="Explain Like I'm 5">
        ...
        <div className="w-full lg:col-span-2 space-y-6 md:space-y-8 flex flex-col">
          {personalNotes.length === 0 ? (
            <div className="w-full text-center py-20 bg-gray-900/30 border border-dashed border-gray-800 rounded-3xl">
            ...
            personalNotes.map(note => (
              <div key={note.id} className="w-full bg-gray-900/80 border border-gray-800 p-6 md:p-8 rounded-3xl hover:border-pink-500/40 transition-colors shadow-xl">
                <div className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 border-b border-gray-800 pb-4">
  ...
  const renderVault = () => (
    <div className="w-full animate-fade-in">
      <div className="w-full flex flex-wrap justify-between items-start sm:items-end gap-6 mb-10 border-b border-gray-800 pb-6">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
        {["Mathematics Matrix", "Physics Core PYQ", "Chemistry Synthesis", "Bio Systems Map", "Historical Timelines", "CS Algorithm Logs"].map((doc, i) => (
          <div key={i} className="w-full group relative cursor-pointer flex flex-col min-h-[12rem] md:min-h-[14rem]">
             ...
             <div className="w-full relative flex-1 bg-gray-900/80 border border-gray-800 group-hover:border-blue-500/50 rounded-3xl p-6 flex flex-col items-center justify-center text-center transition-all duration-300 group-hover:-translate-y-2 shadow-xl backdrop-blur-md">
              <div className="text-4xl md:text-5xl mb-4 group-hover:scale-110 transition-transform duration-300 drop-shadow-md">📄</div>
              <h4 className="w-full break-words font-bold text-gray-200 text-base md:text-lg leading-tight mb-4">{doc}</h4>
            ...
            {nodes.map(node => {
              ...
              return (
                <div key={node.id} className={`w-full group relative flex flex-col min-h-[14rem] md:min-h-[16rem] ${!unlocked ? 'opacity-70' : ''}`}>
                  <div className={`w-full relative flex-1 bg-gray-900/80 border rounded-3xl p-6 md:p-8 overflow-hidden transition-all duration-300 flex flex-col ${unlocked ? 'border-gray-700 hover:border-yellow-500/50 shadow-xl hover:-translate-y-2' : 'border-gray-800'}`}>
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8">
        {channels.map((ch, i) => (
          <div key={i} className="w-full group relative cursor-pointer">
            <div className="w-full bg-gray-900/80 border border-gray-800 rounded-3xl p-5 md:p-8 flex items-center gap-4 md:gap-6 transition-all duration-300 hover:border-red-500/50 hover:-translate-y-1 md:hover:-translate-y-2 shadow-xl backdrop-blur-md" onClick={()=>window.open(ch.url)}>
  ...
  const renderAdmin = () => (
    <div className="w-full max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="w-full bg-gray-900/80 border border-gray-800 rounded-3xl overflow-hidden shadow-2xl backdrop-blur-xl">
        <div className="w-full flex flex-col sm:flex-row border-b border-gray-800">
          <button onClick={() => setAdminTab("syllabus")} className={`w-full sm:flex-1 py-4 md:py-6 font-bold text-sm md:text-base transition-all ${adminTab === 'syllabus' ? 'bg-gray-800 text-white border-b-2 sm:border-b-4 border-gray-400' : 'text-gray-500 hover:bg-gray-800/50 hover:text-gray-300'}`}>Inject Syllabus Data</button>
          <button onClick={() => setAdminTab("channels")} className={`w-full sm:flex-1 py-4 md:py-6 font-bold text-sm md:text-base transition-all ${adminTab === 'channels' ? 'bg-gray-800 text-white border-b-2 sm:border-b-4 border-gray-400' : 'text-gray-500 hover:bg-gray-800/50 hover:text-gray-300'}`}>Manage Creator Nodes</button>
        </div>

        <div className="w-full p-6 md:p-10">
          {adminTab === "syllabus" && (
            <form onSubmit={handleAdminSubmit} className="w-full space-y-4 md:space-y-6">
          ...
          {adminTab === "channels" && (
            <div className="w-full text-center py-16 md:py-20 text-gray-500 text-base md:text-xl font-medium">
  ...
  // ==========================================
  return (
    <div className="flex h-screen w-full bg-[#02000a] text-white font-sans selection:bg-purple-500/30 overflow-hidden relative">
      
      {/* GLOBAL STYLES & TAILWIND CSS FIXES */}
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@300;400;500;600;700&display=swap');
        
        /* PERFECT CSS RESETS SO NOTHING COLLAPSES */
        html, body, #root { 
            width: 100%; 
            height: 100%; 
            margin: 0; 
        ...
        /* FIXED SPACE GRID: Animating background-position instead of transform prevents empty borders */
        .space-grid {
          position: fixed;
      ...
      <div className="absolute inset-0 z-[-1] w-full h-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-900/20 via-[#02000a] to-[#02000a] pointer-events-none"></div>

      {/* 🧭 SIDEBAR NAVIGATION (FLEX ITEM) */}
      {!isSidebarHidden && (
        <aside className="w-72 shrink-0 bg-gray-900/80 backdrop-blur-2xl border-r border-gray-800/80 z-[100] flex flex-col h-full absolute lg:relative transition-transform duration-300">
          {/* Brand */}
          <div className="p-6 border-b border-gray-800/80 flex items-center justify-between shrink-0">
            ...
            <button className="lg:hidden text-gray-400 text-2xl hover:text-white" onClick={() => setIsSidebarHidden(true)}>✕</button>
          </div>
          ...
          {/* Navigation Links */}
          <div className="flex-1 overflow-y-auto px-4 py-2 space-y-1 scroll-smooth w-full">
            {NAV_SYSTEMS.map(sys => (
              <button 
                  key={sys.id} 
      ...
      {isSidebarHidden && (
        <button 
          ...
          className="absolute top-4 md:top-6 left-4 md:left-6 z-[60] w-10 h-10 md:w-12 md:h-12 bg-gray-900/80 backdrop-blur-xl border border-gray-700/50 text-white rounded-xl flex items-center justify-center hover:border-indigo-500/50 transition-colors shadow-2xl text-lg md:text-xl">
          ☰
        </button>
      )}

      {/* 🚀 MAIN CONTENT AREA (FLEX-1 AUTO SCROLL) */}
      <main id="main-scroll-area" className="flex-1 w-full h-full overflow-y-auto overflow-x-hidden relative scroll-smooth">
        <div className="w-full mx-auto px-4 sm:px-6 lg:px-12 pt-20 lg:pt-12 pb-32">
